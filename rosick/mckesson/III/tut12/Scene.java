...
public abstract class Scene {
	...
	class MaterialBlock {
		...
		static final int SIZE = 48;													// Bytes
	...
	Scene(String basepath) {
		...
		for (int mtl = 0; mtl < materials.size(); mtl++) {
			byte temp[] = materials.get(mtl).get();
			
			System.arraycopy(temp, 0, mtlBuffer, mtl * m_sizeMaterialBlock, MaterialBlock.SIZE);
		}

		ByteBuffer buffer = BufferUtils.createByteBuffer(mtlBuffer.length);
		buffer.put(mtlBuffer);
		buffer.flip();
		...
		//glBufferData(GL_UNIFORM_BUFFER, sizeMaterialUniformBuffer, GL_STATIC_DRAW);
