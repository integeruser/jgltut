...
public class DoubleProjection01 extends LWJGLWindow {
	...
	public static void main(String[] args) {
		...
		new DoubleProjection01().start(displayWidth, displayHeight);
	}
		...
		while (Mouse.next()) {
			...
			if (eventButton != -1) {
				if (Mouse.getEventButtonState()) {
					...
					MousePole.forwardMouseButton(persViewPole, eventButton, true, Mouse.getX(), Mouse.getY());			
				} else {
					// Mouse up
					MousePole.forwardMouseButton(viewPole, eventButton, false, Mouse.getX(), Mouse.getY());			
					MousePole.forwardMouseButton(persViewPole, eventButton, false, Mouse.getX(), Mouse.getY());			
				}
			} else {
