...
public class WorldWithUBO02 extends GLWindow {
	...
	protected void display() {
...

		{
			...
			glBindBuffer(GL_UNIFORM_BUFFER, g_GlobalMatricesUBO);
			glBufferSubData(GL_UNIFORM_BUFFER, MAT_SIZE, camMatrix.top().fillBuffer(tempSharedBuffer));
			glBindBuffer(GL_UNIFORM_BUFFER, 0);
	...
	protected void reshape(int width, int height) {
		...
		glBindBuffer(GL_UNIFORM_BUFFER, g_GlobalMatricesUBO);
		glBufferSubData(GL_UNIFORM_BUFFER, 0, persMatrix.top().fillBuffer(tempSharedBuffer));
		glBindBuffer(GL_UNIFORM_BUFFER, 0);
	}
