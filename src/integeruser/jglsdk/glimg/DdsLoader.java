...
public class DdsLoader {
    ...
    private static OldDdsFormatConv oldFormatConvert[] = {
            new OldDdsFormatConv(
                    new ImageFormat.UncheckedImageFormat(ImageFormat.PixelDataType.NORM_UNSIGNED_INTEGER, ImageFormat.PixelComponents.COLOR_RGBA, ImageFormat.ComponentOrder.RGBA, ImageFormat.BitDepth.PER_COMP_8, 1),
                    new OldDdsFmtMatch(DdsPixelFormatFlags.RGB | DdsPixelFormatFlags.ALPHAPIXELS, 32, 0xff, 0xff00, 0xff0000, 0xff000000, 0)
            ),

            new OldDdsFormatConv(
                    new ImageFormat.UncheckedImageFormat(ImageFormat.PixelDataType.NORM_UNSIGNED_INTEGER, ImageFormat.PixelComponents.COLOR_RGBA, ImageFormat.ComponentOrder.BGRA, ImageFormat.BitDepth.PACKED_32_BIT_8888_REV, 1),
                    new OldDdsFmtMatch(DdsPixelFormatFlags.RGB | DdsPixelFormatFlags.ALPHAPIXELS, 32, 0xff0000, 0xff00, 0xff, 0xff000000, 0)
            ),

            new OldDdsFormatConv(
                    new ImageFormat.UncheckedImageFormat(ImageFormat.PixelDataType.NORM_UNSIGNED_INTEGER, ImageFormat.PixelComponents.COLOR_RGBX, ImageFormat.ComponentOrder.RGBA, ImageFormat.BitDepth.PER_COMP_8, 1),
                    new OldDdsFmtMatch(DdsPixelFormatFlags.RGB, 32, 0xff, 0xff00, 0xff0000, 0, 0)
            ),

            new OldDdsFormatConv(
                    new ImageFormat.UncheckedImageFormat(ImageFormat.PixelDataType.NORM_UNSIGNED_INTEGER, ImageFormat.PixelComponents.COLOR_RGB, ImageFormat.ComponentOrder.RGBA, ImageFormat.BitDepth.PER_COMP_8, 1),
                    new OldDdsFmtMatch(DdsPixelFormatFlags.RGB, 24, 0xff, 0xff00, 0xff0000, 0, 0)
            ),
            ...
            new OldDdsFormatConv(
                    new ImageFormat.UncheckedImageFormat(ImageFormat.PixelDataType.NORM_UNSIGNED_INTEGER, ImageFormat.PixelComponents.COLOR_RGB, ImageFormat.ComponentOrder.BGRA, ImageFormat.BitDepth.PER_COMP_8, 1),
                    new OldDdsFmtMatch(DdsPixelFormatFlags.RGB, 24, 0xff0000, 0xff00, 0xff, 0, 0)
            ),

            new OldDdsFormatConv(
                    new ImageFormat.UncheckedImageFormat(ImageFormat.PixelDataType.NORM_UNSIGNED_INTEGER, ImageFormat.PixelComponents.COLOR_RGB, ImageFormat.ComponentOrder.RGBA, ImageFormat.BitDepth.PACKED_16_BIT_565, 1),
                    new OldDdsFmtMatch(DdsPixelFormatFlags.RGB, 16, 0xf800, 0x7e0, 0x1f, 0, 0)
            ),

            new OldDdsFormatConv(
                    new ImageFormat.UncheckedImageFormat(ImageFormat.PixelDataType.NORM_UNSIGNED_INTEGER, ImageFormat.PixelComponents.COLOR_RGBA, ImageFormat.ComponentOrder.BGRA, ImageFormat.BitDepth.PACKED_16_BIT_1555_REV, 1),
                    new OldDdsFmtMatch(DdsPixelFormatFlags.RGB, 16, 0x7c00, 0x3e0, 0x1f, 0x8000, 0)
            ),

            new OldDdsFormatConv(
                    new ImageFormat.UncheckedImageFormat(ImageFormat.PixelDataType.NORM_UNSIGNED_INTEGER, ImageFormat.PixelComponents.COLOR_RGBA, ImageFormat.ComponentOrder.BGRA, ImageFormat.BitDepth.PACKED_16_BIT_4444_REV, 1),
                    new OldDdsFmtMatch(DdsPixelFormatFlags.RGB, 16, 0xf00, 0xf0, 0xf, 0xf000, 0)
            ),

            new OldDdsFormatConv(
                    new ImageFormat.UncheckedImageFormat(ImageFormat.PixelDataType.COMPRESSED_BC1, ImageFormat.PixelComponents.COLOR_RGB, ImageFormat.ComponentOrder.COMPRESSED, ImageFormat.BitDepth.COMPRESSED, 1),
                    new OldDdsFmtMatch(DdsPixelFormatFlags.FOURCC, 0, 0, 0, 0, 0, MagicNumbers.DDSFOURCC_DXT1)
            ),

            new OldDdsFormatConv(
                    new ImageFormat.UncheckedImageFormat(ImageFormat.PixelDataType.COMPRESSED_BC2, ImageFormat.PixelComponents.COLOR_RGBA, ImageFormat.ComponentOrder.COMPRESSED, ImageFormat.BitDepth.COMPRESSED, 1),
                    new OldDdsFmtMatch(DdsPixelFormatFlags.FOURCC, 0, 0, 0, 0, 0, MagicNumbers.DDSFOURCC_DXT3)
            ),

            new OldDdsFormatConv(
                    new ImageFormat.UncheckedImageFormat(ImageFormat.PixelDataType.COMPRESSED_BC3, ImageFormat.PixelComponents.COLOR_RGBA, ImageFormat.ComponentOrder.COMPRESSED, ImageFormat.BitDepth.COMPRESSED, 1),
                    new OldDdsFmtMatch(DdsPixelFormatFlags.FOURCC, 0, 0, 0, 0, 0, MagicNumbers.DDSFOURCC_DXT5)
            ),

            new OldDdsFormatConv(
                    new ImageFormat.UncheckedImageFormat(ImageFormat.PixelDataType.NORM_UNSIGNED_INTEGER, ImageFormat.PixelComponents.COLOR_RG, ImageFormat.ComponentOrder.RGBA, ImageFormat.BitDepth.PER_COMP_16, 1),
                    new OldDdsFmtMatch(DdsPixelFormatFlags.RGB, 32, 0xffff, 0xffff0000, 0, 0, 0)
            ),

            new OldDdsFormatConv(
                    new ImageFormat.UncheckedImageFormat(ImageFormat.PixelDataType.NORM_UNSIGNED_INTEGER, ImageFormat.PixelComponents.COLOR_RG, ImageFormat.ComponentOrder.RGBA, ImageFormat.BitDepth.PER_COMP_8, 1),
                    new OldDdsFmtMatch(DdsPixelFormatFlags.RGB, 16, 0xffff, 0xffff0000, 0, 0, 0)
            ),

            new OldDdsFormatConv(
                    new ImageFormat.UncheckedImageFormat(ImageFormat.PixelDataType.NORM_UNSIGNED_INTEGER, ImageFormat.PixelComponents.COLOR_RED, ImageFormat.ComponentOrder.RGBA, ImageFormat.BitDepth.PER_COMP_16, 1),
                    new OldDdsFmtMatch(DdsPixelFormatFlags.LUMINANCE, 16, 0xffff, 0, 0, 0, 0)
            ),

            new OldDdsFormatConv(
                    new ImageFormat.UncheckedImageFormat(ImageFormat.PixelDataType.NORM_UNSIGNED_INTEGER, ImageFormat.PixelComponents.COLOR_RED, ImageFormat.ComponentOrder.RGBA, ImageFormat.BitDepth.PER_COMP_8, 1),
                    new OldDdsFmtMatch(DdsPixelFormatFlags.LUMINANCE, 8, 0xff, 0, 0, 0, 0)
            ),

            new OldDdsFormatConv(
                    new ImageFormat.UncheckedImageFormat(ImageFormat.PixelDataType.NORM_UNSIGNED_INTEGER, ImageFormat.PixelComponents.COLOR_RG, ImageFormat.ComponentOrder.RGBA, ImageFormat.BitDepth.PER_COMP_16, 1),
                    new OldDdsFmtMatch(DdsPixelFormatFlags.LUMINANCE | DdsPixelFormatFlags.ALPHAPIXELS, 16, 0xffff, 0, 0, 0xffff0000, 0)
            ),

            new OldDdsFormatConv(
                    new ImageFormat.UncheckedImageFormat(ImageFormat.PixelDataType.NORM_UNSIGNED_INTEGER, ImageFormat.PixelComponents.COLOR_RG, ImageFormat.ComponentOrder.RGBA, ImageFormat.BitDepth.PER_COMP_8, 1),
                    new OldDdsFmtMatch(DdsPixelFormatFlags.LUMINANCE | DdsPixelFormatFlags.ALPHAPIXELS, 8, 0xff, 0, 0, 0xff00, 0)
            )
    };
    ...
    private static class OldDdsFmtMatch {
        ...
        OldDdsFmtMatch(int flags, int bitDepth, int rBitmask, int gBitmask, int bBitmask, int aBitmask, int fourCC) {
            this.flags = flags;
    ...
    private static boolean doesMatchFormat(OldDdsFmtMatch ddsFmt, DdsHeader ddsHeader) {
        if ((ddsHeader.ddsPixelFormat.flags & ddsFmt.flags) == 0) return false;

        if ((ddsFmt.flags & DdsPixelFormatFlags.FOURCC) != 0) {
            // None of the bit counts matter. Just check the fourCC
            if (ddsFmt.fourCC != ddsHeader.ddsPixelFormat.fourCC) return false;
        } else {
            // Check the bitcounts, not the fourCC.
            if (ddsHeader.ddsPixelFormat.rgbBitCount != ddsFmt.bitDepth) return false;
            if ((ddsFmt.rBitmask & ddsHeader.ddsPixelFormat.rBitMask) != ddsFmt.rBitmask) return false;
            if ((ddsFmt.gBitmask & ddsHeader.ddsPixelFormat.gBitMask) != ddsFmt.gBitmask) return false;
            if ((ddsFmt.bBitmask & ddsHeader.ddsPixelFormat.bBitMask) != ddsFmt.bBitmask) return false;
            if ((ddsFmt.aBitmask & ddsHeader.ddsPixelFormat.aBitMask) != ddsFmt.aBitmask) return false;
        }
