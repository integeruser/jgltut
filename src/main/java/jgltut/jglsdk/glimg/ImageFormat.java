...
class ImageFormat {
    ...
    static class UncheckedImageFormat {
        ...
        UncheckedImageFormat(PixelDataType type, PixelComponents format, ComponentOrder order, BitDepth bitdepth, int lineAlignment) {
            this.type = type;
            this.format = format;
            this.order = order;
    ...
    int alignByteCount(int byteCount) {
        return (byteCount + (uncheckedImageFormat.lineAlignment - 1)) / uncheckedImageFormat.lineAlignment;
    }
