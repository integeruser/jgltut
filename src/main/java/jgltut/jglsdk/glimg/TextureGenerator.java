...
import jgltut.jglsdk.glimg.ImageFormat.BitDepth;
import jgltut.jglsdk.glimg.ImageFormat.ComponentOrder;
import jgltut.jglsdk.glimg.ImageFormat.PixelComponents;
import jgltut.jglsdk.glimg.ImageFormat.PixelDataType;
...
public class TextureGenerator {
    ...
    private static PixelDataType getDataType(ImageFormat imageFormat, int forceConvertBits) {
        ...
        if (!forceIntegral && !forceSigned) return imageFormat.getPixelDataType();
        ...
        switch (imageFormat.getPixelDataType()) {
            ...
            case UNSIGNED_INTEGRAL:
                if (forceSigned) return PixelDataType.SIGNED_INTEGRAL;

            case COMPRESSED_UNSIGNED_BC4:
                if (forceSigned) return PixelDataType.COMPRESSED_SIGNED_BC4;
            case COMPRESSED_UNSIGNED_BC5:
                if (forceSigned) return PixelDataType.COMPRESSED_SIGNED_BC5;
        }
    ...
    private static void throwIfLANotSupported() {
        if (!GLContext.getCapabilities().OpenGL30) return;
        ...
        switch (pixelDataType) {
            ...
            case FLOAT: {
                ...
                if (imageFormat.getBitDepth().ordinal() < BitDepth.NUM_PER_COMPONENT.ordinal()) {
                    ...
                    return throwInvalidFormatIfZero(floatFormats[(2 * imageFormat.getPixelComponents().ordinal()) + offset]);
                } else {
    ...
    private static boolean isTextureStorageSupported() {
        if (!GLContext.getCapabilities().OpenGL42) {
            if (!GLContext.getCapabilities().GL_ARB_texture_storage) return false;
        }
        return true;
    }

    private static boolean isDirectStateAccessSupported() {
        if (!GLContext.getCapabilities().GL_EXT_direct_state_access) return false;
        return true;
    }


    private static void build2DTexture(int textureName, ImageSet imageSet, int forceConvertBits, int internalFormat,
        ...
        texStorageBase(GL_TEXTURE_2D, forceConvertBits, imageSet.getDimensions(), numMipmaps, internalFormat, upload, textureName);
        ...
        for (int mipmapLevel = 0; mipmapLevel < numMipmaps; mipmapLevel++) {
            ...
            texSubImage(textureName, GL_TEXTURE_2D, mipmapLevel, internalFormat, imageDimensions, upload, imageDataBuffer, imageSet.getSize(mipmapLevel));
        }
    ...
    // DSA-style
    private static void manTexStorageBase(int texture, int textureTarget, Dimensions imageDimensions, int numMipmaps,
    ...
    private static void texSubImage(int texture, int textureTarget, int mipmapLevel, int internalFormat, Dimensions imageDimensions,
        ...
        switch (imageDimensions.numDimensions) {
            case 1:
                if (upload.blockByteCount != 0) {
                    glCompressedTexSubImage1D(textureTarget, mipmapLevel, 0, imageDimensions.width, internalFormat, imageData);
                } else {
                    glTexSubImage1D(textureTarget, mipmapLevel, 0, imageDimensions.width, upload.format, upload.type, imageData);
                }
                break;
            ...
            case 2:
                if (upload.blockByteCount != 0) {
                    ...
                    glTexSubImage2D(textureTarget, mipmapLevel, 0, 0, imageDimensions.width, imageDimensions.height,
            ...
            case 3:
                if (upload.blockByteCount != 0) {
                    glCompressedTexSubImage3D(textureTarget, mipmapLevel, 0, 0, 0, imageDimensions.width,
                            imageDimensions.height, imageDimensions.depth, internalFormat, imageData);
                } else {
                    glTexSubImage3D(textureTarget, mipmapLevel, 0, 0, 0, imageDimensions.width, imageDimensions.height,
